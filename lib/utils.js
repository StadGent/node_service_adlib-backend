...
module.exports = new class Utils {
    async insertObject(institution, db, object, adlibDatabase, correlator) {
        ...
        this.log(`Inserting object with URI ${URI}`, "adlib-backend/lib/utils.js:insertObject", correlator.getId())
    ...
    async initDb(correlator) {
        ...
        try {
          ...
          console.log(`[Database] ${err.message} Retrying...`);
          await this.sleep(3000);
          return this.initDb(correlator);
        }
        ...
        if (!process.env.npm_package_version) {
            this.log("EMPTYING DATABASE", "adlib-backend/lib/utils.js:initDb", correlator.getId())
    ...
    log(message, loggerName, correlationId) {
        console.log(`{"@timestamp":"${new Date().toISOString()}","@version":1,"message":"${message}","logger_name":"${loggerName}","level":"INFO","level_value":"1","correlationId":"${correlationId}"}`);
    }
}